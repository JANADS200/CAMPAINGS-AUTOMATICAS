...
export const executeFinalDeployment = async (
  ...
  try {
    ...
    for (const [sIdx, struct] of strategy.structure.entries()) {
      ...
      for (const [aIdx, asset] of assets.entries()) {
        ...
        if (asset.type === 'image') adStorySpec.link_data.picture = asset.url;
        ...
        if (!crData.error) {
          ...
          await fetch(`https://graph.facebook.com/${API_VER}/${adAccountId}/ads`, { method: 'POST', body: adParams });
        }
      }
    }
